...
Container.prototype.loadStreamTarget = function(etcd, done){
	if(this._job.streamTarget){
		return done(null, this._job.streamTarget)
	}
	else{
		Endpoints.leader(etcd, function(err, endpoint){
			if(err) return done(err)
			done(null, endpoint + '/v1/feedback')
		})
	}
}
