...
Container.prototype.stream = function(){
	...
	var runOptions = self.options()
	runOptions.deamon = false
	runOptions.deamonize = false
	runOptions.stdio = 'pipe'

	return Dockerrun(runOptions)
}
...
Container.prototype.start = function(opts, callback){
	...
	function start(){
		var runOptions = self.options()
		runOptions.deamon = true
		Dockerrun(runOptions, function(){
			self.emit('started')
