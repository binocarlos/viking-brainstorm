...
VikingFile.prototype.load = function(done){
	...
	var requires = this.viking.require || []
	...
	requires.forEach(function(stack){
		requirestacks[stack] = true
	})
	...
	Object.keys(containers || {}).forEach(function(key){
		...
		if(container.link){
			container.link.forEach(function(link){
				...
				// external link - add to the stack deps
				if(link.indexOf('/')>0){
				...
				// internal link - promote what it links to up the batch order
				else{
					linkDeps[link] = true
				}
	...
	// the ones that are providing env
	var preBootOrder = bootOrder.filter(function(name){
		return linkDeps[name]
	})

	// the ones that need env
	var postBootOrder = bootOrder.filter(function(name){
		return !linkDeps[name]
	})

	var data = {
		requirestacks:requirestacks,
		buildOrder:buildOrder,
		bootOrder:[preBootOrder, postBootOrder],
		images:imagePojos,
		imageDeps:imageDeps,
