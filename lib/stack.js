...
function Stack(vikingconfig, dir, opts){
	...
	this.builders = {}
	this.deployment = Deployment(vikingconfig, this.etcd)
}
...
Stack.prototype.load = function(done){
	...
	async.series([
	  ...
	  function(next){
			self.vikingfile.load(self.etcd, function(){
	...
	async.series([
		...
		function(next){
			...
			async.forEachSeries(self._buildOrder, function(name, nextImage){
				...
				var builder = self.builders[name]
				self.deployment.saveImage(builder.saveImageName(), builder.remoteName(), nextImage)
...
Stack.prototype.deploy = function(options, done){
	console.log('-------------------------------------------');
	console.log('-------------------------------------------');
	console.dir(options)
	return done()
