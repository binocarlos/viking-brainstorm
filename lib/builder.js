...
Builder.prototype.localName = function(splitter){
	return images.localName(this.stack, this.tag, this.node, splitter)
}
...
Builder.prototype.remoteName = function(tag){
	var self = this;
	return images.remoteName(this.registry, this.stack, tag || this.tag, this.node)
}
...
Builder.prototype.build = function(done){
	...
	logger('[dev build] ' + this.localName() + ' start')
	...
	dockerBuild.on('close', function(code){
		...
		logger('[dev build] ' + self.stack + '/' + self.node + ' done')
...
Builder.prototype.tagImage = function(tag, done){
	var self = this;
	logger('[tag] ' + self.stack + '/' + self.node)
	...
	dockerTag.on('close', function(code){
		...
		logger('[push] ' + self.stack + '/' + tag + ' - ' + self.node)
		return done()
	})
}

Builder.prototype.push = function(tag, done){

	var self = this;
	async.series([
	  function(next){
	  	self.tagImage(tag, next)
	  },
	  ...
	  function(next){
			...
			dockerPush.on('close', function(code){
				if(code!=0){
					return next('docker push for ' + self.remoteName(tag) + ' returned a non-zero exit code')
				}
				return next()
			})
	  }
