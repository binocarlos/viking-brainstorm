...
Builder.prototype.build = function(done){
	...
	utils.log('dev build ' + this.stack + '/' + this.node + ':' + this.tag, 'start')
	...
	var tag = this.stack + '-' + this.node// + ':' + this.tag
	...
	dockerBuild.on('close', function(code){
		if(code!=0){
			return done('docker build for ' + self.stack + '/' + self.node + ' returned a non-zero exit code')
		}
