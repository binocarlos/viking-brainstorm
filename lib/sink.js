...
module.exports = function(config, etcd){
	...
	function runJob(job, done){
		...
		deployment.canRunJob(job, hostname, function(err, canRun){
			...
			console.log('-------------------------------------------');
			console.log('-------------------------------------------');
			console.log('RUNNING CONTAINBER')
			...
			container.start(function(err, data){
				...
				console.log('-------------------------------------------');
				console.log('-------------------------------------------');
				console.log('CONTAINE STARTED')

				etcd.set('/deploy/' + hostname + jobObject.key(), job.stack + '-' + job.name, done)
			})
		})
	...
	function checkDeployment(){
		...
		async.parallel({
			...
			async.forEach(Object.keys(data.deploy || {}), function(deployPath, nextContainer){
				...
				function processJob(){
					console.log('-------------------------------------------');
					console.log('process : ' + containerPath)
