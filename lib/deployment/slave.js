...
module.exports = function(config, etcd){
	...
	return {
		...
		canRunJobCounter:function canRunJobCounter(job, hostname, done){
			var self = this;
			if(job.system){
		...
		writeContainer:function writeContainer(opts, done){
			var self = this
			var job = opts.job
			var ip = opts.ip
			...
			async.parallel([
				...
				function(next){
					etcd.set('/deploy/' + hostname + jobObject.key(), jobObject.containerName()job.stack + '-' + job.tag + '-' + job.name + '-' + job.pid, next)
				},
		...
		runSlaveContainers:function runSlaveContainers(data, done){
			...
			async.forEach(Object.keys(data.deploy || {}), function(deployPath, nextContainer){
				...
				function processJob(){
					...
					self.loadJob(nameParts.stack, nameParts.tag, nameParts.name, function(err, jobData){
						if(err || !jobData){
