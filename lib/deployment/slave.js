...
module.exports = function(config, etcd){
	...
	function writeContainer(opts, done){
		...
		logger('[slave write container] ' + job.id + ' [json] ' + JSON.stringify(container))
	...
	function slaveRunJob(job, done){
		...
		canRunJobCounter(job, hostname, function(err, canRun){
			if(!canRun){
			...
			logger('[slave run job] [json] ' + JSON.stringify(job, null, 4))
	...
	function removeSlaveContainers(data, done){
		...
		async.forEach(Object.keys(data.docker || {}), function(containerName, nextContainer){
			...
			logger('[remove] ' + containerName + ' [json] ' + JSON.stringify(data))
