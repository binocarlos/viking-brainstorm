...
module.exports = function(config, etcd){
	...
	function slaveRunJob(job, done){
		...
		container.on('stream', function(stream){
			...
			if(!streamTarget){
				logger.error('[no stream target]')
				return
			}

			var url = 'http://' + streamTarget + jobObject.key()
		...
		async.series([
		  function(next){
				container.prepare(config, next)
		  },

		  function(next){
				container.loadStreamTarget(etcd, function(err, target){
