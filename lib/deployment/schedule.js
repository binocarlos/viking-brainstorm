...
module.exports = function(config, etcd){
	...
	function writeJob(job, done){
		...
		async.forEachSeries(jobs, function(runJob, nextJob){
			var key = tools.jobKey('proc', runJob)
			logger('[schedule write] ' + runJob.id + ' ' + key + ' [json] ' + JSON.stringify(runJob))
			
			etcd.set(key, JSON.stringify(runJob), nextJob)
		}, done)
