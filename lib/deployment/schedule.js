...
module.exports = function(config, etcd){
	...
	function writeJob(job, done){
		...
		while(scale>0){
			...
			console.log('[schedule write] ' + scaleJob.id + ' [json] ' + JSON.stringify(scaleJob))
			jobs.push(scaleJob)
		}
	...
	function runJob(job, server, done){
		...
		console.log('[schedule run] ' + job.id + ' [json] ' + JSON.stringify({
			job:scaleJob,
			server:server.name
		}))
	...
	function writeEndpoint(job, opts, done){
		...
		console.log('[schedule endpoint] ' + job.id + ' [json] ' + JSON.stringify(opts))
	...
	function writeDockerEndpoint(settings, done){
		...
		console.log('[schedule docker endpoint] [json] ' + JSON.stringify(settings))
	...
	function removeJob(opts, done){
		...
		console.log('[remove job] ' + jobObject.key() + ' [json] ' + JSON.stringify(opts))
