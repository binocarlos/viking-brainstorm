...
module.exports = function(config, etcd){
	...
	function processJobImage(stack, job, done){
		...
		if(job.image.indexOf('viking:')==0){
			...
			job.image = images.remoteName(stack.registry, job.stack, job.tag, job.name)
			done()
		}
		else{
			done()
	...
	function manageDeployment(stack, job, done){
		...
		prepareJob(stack, job, function(err){
			if(err) return done(err)
	...
	function expandBatch(stack, batch){
    var jobs = []
    ...
    async.forEachSeries([stack.getBatches()[1]], function(batch, nextBatch){

      logger('[run batch] ' + stack.getId() + ' - ' + batch.length + ' jobs')

      var jobs = expandBatch(stack, batch)

      deployBatch(stack, jobs, function(err, data){

        console.log('-------------------------------------------');
        console.log('-------------------------------------------');
        console.log('deployed batch')
        console.dir(data)

/*
        var results = jobs.map(function(job, i){
          return {
            job:job,
            data:data
          }
