...
function feedback(etcd, done){
	leader(etcd, function(err, leader){
		if(err) return done(err)
		done(null, 'http://' + leader + '/v1/feedback')
	})
}


function jobFeedbackTarget(etcd, job, done){
	var jobObject = Job(job)
	if(job.feedbackTarget){
		done(null, job.feedbackTarget)
	}
	else{
		feedback(etcd, function(err, feedback){
			if(err) return done(err)
			done(null, feedback + jobObject.key())
		})
	}
}
...
module.exports = {
	leader:leader,
	feedback:feedback,
	jobFeedbackTarget:jobFeedbackTarget,
	registry:registry,
	endpoint:endpoint,
	node:node,
