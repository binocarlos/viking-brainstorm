	...
	function getServerJobs(hostname, done){
		...
		etcd.get('/deploy/' + hostname, {
			...
			async.forEachSeries(Object.keys(jobs), function(jobid, nextJob){
				...
				var methodname = 'loadJob'

				// this accounts for if the job has failed
				if(!job){
					job = jobid.replace('/deploy/' + hostname, '')
					methodname = 'loadJobByPath'
				}
				
				schedule[methodname].apply(schedule, [job, function(err, jobObject){
