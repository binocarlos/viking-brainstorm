...
// convert the image ADD commands into volumes for the local folder
VikingFile.prototype.development = function(folder){
	var self = this;
	...
	Object.keys(imgs || {}).forEach(function(key){
		...
		(image.add || []).forEach(function(add){
			add.source = add.source.replace(/^\./, folder)
			image.volume.push(add.source + ':' + add.dest)
		})
		image.add = []
	})
}
