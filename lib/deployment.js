...
module.exports = function(config, etcd){
	...
	deployment.writeEndpoint = function(job, opts, done){
		...
		if(opts.ports && opts.ports.length){
			...
			var portKey = '/ports/' + job.stack + '/' + job.name + '/' + job.tag + '/' + containerPort + '/' + proto + '/' + ip + '/' + hostPort
	...
	deployment.writeContainer = function(opts, done){
		...
		async.parallel([
			...
			function(next){
				etcd.set('/deploy/' + hostname + jobObject.key(), job.stack + '-' + job.name + '-' + job.tag, done)
			}
		], done)
	}
	...
	deployment.getJobState = function(job, done){
		...
		this.getJobServer(job, function(err, server){
			...
			async.parallel({
				...
				ports:function(next){
					etcd.get('/ports' + jobObject.portKey(), {
						recursive:true
	...
	deployment.removeJob = function(opts, done){
		...
		async.parallel({
			ports:function(next){
				etcd.get('/ports' + jobObject.portKey(), function(err, portData){
					if(err) return next(err)
			...
			Object.keys(data.ports || {}).forEach(function(key){
				...
				if(key.indexOf(jobObject.portKey() + '/' + job.tag)>=0){
					keys.push(key)	
				}
			})
